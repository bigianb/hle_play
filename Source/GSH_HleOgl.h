...
#define PREF_CGSH_HleOgl_LINEASQUADS				"renderer.hle.opengl.linesasquads"
...
public:
	...
	*/
	...
	virtual void	DrawSprite(int xpos, int ypos, int width, int height, uint32 vertexRGBA, uint8* texGsPacketData, bool interlaced, uint64 alphaReg) override {};
...
private:
	...
	typedef void (CGSH_HleOgl::*TEXTUREUPLOADER)(uint32, uint32, unsigned int, unsigned int);
	typedef void (CGSH_HleOgl::*TEXTUREUPDATER)(uint32, uint32, unsigned int, unsigned int, unsigned int, unsigned int);
